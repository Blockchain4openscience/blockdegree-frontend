...
`````
composer-rest-server  -c admin@blockdegrees -n never -p 3001
`````
use `admin@blockdegrees` as the card name.
...
`````
`````
composer-rest-server -c admin@blockdegrees -n never -a true -m true -w true
`````
use `admin@blockdegrees` as the card name.
